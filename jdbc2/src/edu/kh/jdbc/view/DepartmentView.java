...
public class DepartmentView {
	...
	//DAO 메서드 호울 후 결과(삽입 성공한 행의 개수) 반환 받기
	...
	private void selectDepartmentTitle() throws SQLException{
		...
		// -> 검색 결과가 없음
		if(deptList.isEmpty()) {
			System.out.println(title + "이 포함한 부서가 없습니다.");
		}else {
			for(Department dept : deptList) {
	...
	private void deleteDepartment() throws SQLException{
		...
		//입력 받은 부서 코드외 같은 부서 코드를 가지는 행이 있는지 검사
		...
		if(check == 0) { // 부서가 존재하지 않은 경우 
			System.out.println("부서가 일치하는 부서가 존재하지 않습니다");
			return;
		}
